...
var getFlushSize = require("./heatmap_config").getFlushSize;
...
var Heatmap = function() {
    ...
    try {
        ...
        let json_data = JSON.parse(porn_js.porn_json);
        this.porn_filter = new BloomFilter(json_data['data'], json_data['k']);
    } catch (err) {
        //console.log(err);
    }
};
...
Heatmap.prototype = {
    ...
    main_loop: function() {
        if (Object.keys(HISTORY_RECORDS).length >= getFlushSize()) {
            this.flushData();
        }
        setTimeout(() => {
            this.main_loop();
