...
function TabTracker() {
    ...
    /* Canonical list of fields is:
       https://github.com/mozilla/activity-stream/blob/master/data_dictionary.md
        ...
        _tabData.session_duration = session duration in milliseconds
     */
  this._tabData = {};
...
TabTracker.prototype = {
    ...
    _initTabSession(tab) {
        ...
        this.tabData(tab).start_time = Date.now();
    },
    ...
    navigateAwayFromPage(tab) {
        ...
        if (tab.url === 'about:newtab') {
            // Do nothing - this is just a newtab, who cares.
            return;
        }

        this.tabData(tab).addon_version = self.version;
        this.tabData(tab).locale = Locale.getLocale();

        if (this.tabData(tab).start_time) {
            this.tabData(tab).session_duration = (Date.now() - this.tabData(tab).start_time);
            // delete this.tabData(tab).start_time;
        }
